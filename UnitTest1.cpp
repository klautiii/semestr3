...
// ============================================================
// Тесты для метода fillRandom
// ============================================================

TEST(MatrixFillRandomTest, FillRandomInRange) {
    Matrix<int> m(5, 5);
    m.fillRandom(1, 10);
...
TEST(MatrixFillRandomTest, FillRandomMinEqualsMax) {
    Matrix<int> m(3, 3);
    m.fillRandom(5, 5);
...
// ============================================================
// Тесты для метода fillFromStream
// ============================================================

TEST(MatrixFillFromStreamTest, FillFromStringStream) {
    std::stringstream ss;
    ss << "1 2 3 4 5 6";
...
TEST(MatrixFillFromStreamTest, FillWithNegativeNumbers) {
    std::stringstream ss;
    ss << "-1 2 -3 4 -5 6";
...
// ============================================================
// Тесты для метода replaceMinAbsInColumns
// ============================================================

TEST(MatrixReplaceMinAbsTest, BasicCase) {
    Matrix<int> m(3, 3);
    m[0] = { 5, -2, 3 };
    m[1] = { 1, 8, -4 };
    m[2] = { 7, -1, 6 };

    m.replaceMinAbsInColumns();

    // Столбец 0: min abs = 1 (индекс 1)
    EXPECT_EQ(m[0][0], 5);
    EXPECT_EQ(m[1][0], 0);
    EXPECT_EQ(m[2][0], 7);

    // Столбец 1: min abs = 1 (индекс 2)
    EXPECT_EQ(m[0][1], -2);
    EXPECT_EQ(m[1][1], 8);
    EXPECT_EQ(m[2][1], 0);

    // Столбец 2: min abs = 3 (индекс 0)
    EXPECT_EQ(m[0][2], 0);
    EXPECT_EQ(m[1][2], -4);
    EXPECT_EQ(m[2][2], 6);
}

TEST(MatrixReplaceMinAbsTest, AllPositive) {
    Matrix<int> m(2, 3);
    m[0] = { 3, 7, 2 };
    m[1] = { 5, 1, 8 };
    ...
    // Столбец 0: min = 3 (индекс 0)
    EXPECT_EQ(m[0][0], 0);
    EXPECT_EQ(m[1][0], 5);

    // Столбец 1: min = 1 (индекс 1)
    EXPECT_EQ(m[0][1], 7);
    EXPECT_EQ(m[1][1], 0);

    // Столбец 2: min = 2 (индекс 0)
    EXPECT_EQ(m[0][2], 0);
    EXPECT_EQ(m[1][2], 8);
}

TEST(MatrixReplaceMinAbsTest, AllNegative) {
    Matrix<int> m(2, 3);
    m[0] = { -3, -7, -2 };
    m[1] = { -5, -1, -8 };
    ...
    // Столбец 0: min abs = 3 (индекс 0)
    EXPECT_EQ(m[0][0], 0);
    EXPECT_EQ(m[1][0], -5);

    // Столбец 1: min abs = 1 (индекс 1)
    EXPECT_EQ(m[0][1], -7);
    EXPECT_EQ(m[1][1], 0);

    // Столбец 2: min abs = 2 (индекс 0)
    EXPECT_EQ(m[0][2], 0);
    EXPECT_EQ(m[1][2], -8);
}

TEST(MatrixReplaceMinAbsTest, DuplicateMinimum) {
    Matrix<int> m(3, 2);
    m[0] = { 1, 4 };
    m[1] = { 2, 4 };
    m[2] = { 3, 4 };
    ...
    // Столбец 0: min = 1 (индекс 0)
    EXPECT_EQ(m[0][0], 0);
    EXPECT_EQ(m[1][0], 2);
    EXPECT_EQ(m[2][0], 3);

    // Столбец 1: min = 4 (индекс 0) - первый из равных
    EXPECT_EQ(m[0][1], 0);
    EXPECT_EQ(m[1][1], 4);
    EXPECT_EQ(m[2][1], 4);
}

TEST(MatrixReplaceMinAbsTest, SingleRow) {
    Matrix<int> m(1, 4);
    m[0] = { 3, -7, 2, -5 };
...
TEST(MatrixReplaceMinAbsTest, SingleColumn) {
    Matrix<int> m(4, 1);
    m[0] = { 3 };
    m[1] = { -7 };
    m[2] = { 2 };
...
TEST(MatrixReplaceMinAbsTest, ZeroValues) {
    Matrix<int> m(2, 2);
    m[0] = { 0, 5 };
    m[1] = { 3, 0 };
    ...
    // Столбец 0: min abs = 0 (индекс 0)
    EXPECT_EQ(m[0][0], 0);
    EXPECT_EQ(m[1][0], 3);

    // Столбец 1: min abs = 0 (индекс 1)
    EXPECT_EQ(m[0][1], 5);
    EXPECT_EQ(m[1][1], 0);
}

TEST(MatrixReplaceMinAbsTest, EmptyMatrix) {
    Matrix<int> m;
    m.replaceMinAbsInColumns();
    EXPECT_EQ(m.getRows(), 0);
    EXPECT_EQ(m.getCols(), 0);
...
// ============================================================
// Тесты для метода removeColumnsWhereFirstGreaterThanLast
// ============================================================

TEST(MatrixRemoveColumnsTest, BasicCase) {
    Matrix<int> m(2, 4);
    m[0] = { 3, 1, 5, 2 };
    m[1] = { 1, 4, 2, 3 };

    m.removeColumnsWhereFirstGreaterThanLast();

    // Столбец 0: 3 > 1 -> удаляем
    // Столбец 1: 1 > 4 -> нет -> оставляем
    // Столбец 2: 5 > 2 -> удаляем
    // Столбец 3: 2 > 3 -> нет -> оставляем
    EXPECT_EQ(m.getCols(), 2);
    EXPECT_EQ(m[0][0], 1);
    EXPECT_EQ(m[0][1], 2);
    EXPECT_EQ(m[1][0], 4);
...
TEST(MatrixRemoveColumnsTest, AllColumnsRemoved) {
    Matrix<int> m(2, 3);
    m[0] = { 5, 7, 2 };
    m[1] = { 1, 3, 0 };
...
TEST(MatrixRemoveColumnsTest, NoColumnsRemoved) {
    Matrix<int> m(2, 3);
    m[0] = { 1, 2, 3 };
    m[1] = { 2, 4, 5 };
...
TEST(MatrixRemoveColumnsTest, SingleRow) {
    Matrix<int> m(1, 4);
    m[0] = { 1, 2, 3, 4 };

    m.removeColumnsWhereFirstGreaterThanLast();

    // Для одной строки первый == последний -> все столбцы сохраняются
    EXPECT_EQ(m.getCols(), 4);
    EXPECT_EQ(m[0][0], 1);
    EXPECT_EQ(m[0][1], 2);
    EXPECT_EQ(m[0][2], 3);
...
TEST(MatrixRemoveColumnsTest, SingleColumn) {
    Matrix<int> m(3, 1);
    m[0] = { 5 };
    m[1] = { 3 };
    m[2] = { 2 };
...
TEST(MatrixRemoveColumnsTest, NegativeNumbers) {
    Matrix<int> m(2, 3);
    m[0] = { -1, -3, 2 };
    m[1] = { -2, -5, 1 };
    ...
    // Столбец 0: -1 > -2 -> да -> удаляем
    // Столбец 1: -3 > -5 -> да -> удаляем
    // Столбец 2: 2 > 1 -> да -> удаляем
    EXPECT_EQ(m.getRows(), 2);
    EXPECT_EQ(m.getCols(), 0);
}

TEST(MatrixRemoveColumnsTest, EmptyMatrix) {
    Matrix<int> m;
    m.removeColumnsWhereFirstGreaterThanLast();
    EXPECT_EQ(m.getRows(), 0);
    EXPECT_EQ(m.getCols(), 0);
...
// ============================================================
// Тесты для метода replaceFirstThreeColumnsWithSquares
// ============================================================
...
// ============================================================
// Тесты для метода insertFirstRowAfterOddRows
// ============================================================
...
TEST(Task1Test, GetDescription) {
    Task1 task;
    EXPECT_EQ(task.getDescription(), "Replace the minimum absolute value element of each column with zero");
}
...
TEST(Task2Test, GetDescription) {
    Task2 task;
    EXPECT_EQ(task.getDescription(), "Remove all columns where the first element is greater than the last element");
}
...
TEST(ExerciseTest, SetGenerator) {
    Task1 task;
    auto gen = std::make_unique<RandomGenerator>(1, 10);
    task.setGenerator(std::move(gen));
    // Проверяем, что генератор установлен (нет ошибок компиляции)
    SUCCEED();
}
...
TEST(IntegrationTest, FullWorkflowWithTask1) {
    // Создаем матрицу
    Matrix<int> m(3, 3);
    m[0] = { 5, -2, 3 };
    m[1] = { 1, 8, -4 };
    ...
    // Проверяем результат
    EXPECT_EQ(result1[0][0], 5);
    EXPECT_EQ(result1[1][0], 0);
    EXPECT_EQ(result1[2][0], 7);
    EXPECT_EQ(result1[0][1], -2);
    EXPECT_EQ(result1[1][1], 8);
    EXPECT_EQ(result1[2][1], 0);
    EXPECT_EQ(result1[0][2], 0);
    EXPECT_EQ(result1[1][2], -4);
    EXPECT_EQ(result1[2][2], 6);
...
TEST(IntegrationTest, FullWorkflowWithTask2) {
    // Создаем матрицу
    Matrix<int> m(2, 4);
    m[0] = { 3, 1, 5, 2 };
    m[1] = { 1, 4, 2, 3 };
    ...
    task2.setMatrix(m);
    task2.solve();
    Matrix<int> result = task2.getMatrix();
...
TEST(IntegrationTest, MultipleOperations) {
    ...
    // 3. Применяем задание 1
    Task1 task1;
    task1.setMatrix(m);
    task1.solve();
    Matrix<int> result1 = task1.getMatrix();

    // 4. Применяем задание 2
    Task2 task2;
    task2.setMatrix(result1);
    task2.solve();
    Matrix<int> result2 = task2.getMatrix();
...
TEST(StressTest, LargeMatrixReplaceMinAbs) {
    const size_t size = 100;
    Matrix<int> m(size, size);
    m.fillRandom(1, 1000);

    // Просто проверяем, что операция выполняется без ошибок
    EXPECT_NO_THROW(m.replaceMinAbsInColumns());
}

TEST(StressTest, LargeMatrixRemoveColumns) {
    const size_t size = 100;
    Matrix<int> m(size, size);
    m.fillRandom(1, 1000);

    EXPECT_NO_THROW(m.removeColumnsWhereFirstGreaterThanLast());
}
